...
-   **Встроенные сервисы и резервирование одной модели**: Поддерживаются Gemini API, OpenRouter, Cloudflare, Groq, NVIDIA NIM, Hugging Face, Vercel AI Gateway, OpenAI и Anthropic. При отказе сервиса или ограничении частоты запросов приложение пробует следующее подключение только для *той же модели* и не переключается на другую модель без уведомления.
