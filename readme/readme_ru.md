...
-   **Ускорение на GPU (NVIDIA, по желанию)**: При наличии GPU NVIDIA можно один раз загрузить GPU-компоненты (~2 ГБ) из стартового диалога, после чего тегирование будет выполняться на CUDA (~28x быстрее по замерам на RTX 4070). Без загрузки - или на машине без NVIDIA - приложение продолжает работать на CPU как обычно. Создание описаний (Florence-2) всегда выполняется на CPU из-за квантованной модели. Переключается через `[Behavior] onnx_device` в `config.ini` (`auto` / `cpu` / `cuda`).
