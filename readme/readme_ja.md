...
- **内蔵サービスと同一モデルフォールバック**: Gemini API・OpenRouter・Cloudflare Workers AI を内蔵（ほかに Groq・NVIDIA NIM・Mistral）。あるサービスが拒否・レート制限された場合、*同じ*モデルを提供する次のサービスへ自動で切り替えます。別モデルへ勝手に乗り換えることはありません。
...
- **既定は無料経路のみ**: 有料継続を明示的に許可しない限り、アプリが無料と確認できる経路だけを使います。詳細度・文数・キャラクター名の扱い・Markdown を調整でき、生成キャプションは既存 `.txt` と組み合わせられます（前に追加／後に追加／上書き。タグ出力と同じ）。
