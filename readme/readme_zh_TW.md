...
-   **內建服務與同模型故障轉移**: 支援 Gemini API、OpenRouter、Cloudflare、Groq、NVIDIA NIM、Hugging Face、Vercel AI Gateway、OpenAI 和 Anthropic。服務拒絕請求或受到速率限制時，只會嘗試提供*同一模型*的下一條連線，不會在未告知的情況下切換到其他模型。
